...
export class GitRefManager {
  ...
  static async writeRef(
    { fs, gitdir, ref, value }:
    { fs: FileSystem, gitdir: string, ref: string, value: string }) {
    // Validate input
    if (!value.match(/[0-9a-f]{40}/)) {
      throw new InvalidOidError(value)
    }
  ...
  static async deleteRef(
    { fs, gitdir, ref }:
    { fs: FileSystem, gitdir: string, ref: string }) {
    return GitRefManager.deleteRefs({ fs, gitdir, refs: [ref] })
  }

  static async deleteRefs(
    { fs, gitdir, refs }:
    { fs: FileSystem, gitdir: string, refs: string[] }) {
    // Delete regular ref
    await Promise.all(refs.map(ref => fs.rm(join(gitdir, ref))))
    // Delete any packed ref
    let text = (await fs.read(`${gitdir}/packed-refs`, { encoding: 'utf8' })) as string
  ...
  static async resolve(
    { fs, gitdir, ref, depth = undefined }:
    { fs: FileSystem, gitdir: string, ref: string, depth?: number }): Promise<string> {
    if (depth !== undefined) {
      depth--
      if (depth === -1) {
        return ref
    ...
    for (const ref of allpaths) {
      sha =
        (await fs.read(`${gitdir}/${ref}`, { encoding: 'utf8' })) as string ||
        packedMap.get(ref)
      if (sha) {
  ...
  static async expandAgainstMap({ ref, map }: { ref: string, map: Map<string, string> }) {
    // Look in all the proper paths, in this order
    const allpaths = refpaths(ref)
    for (const ref of allpaths) {
  ...
  static resolveAgainstMap(
    { ref, fullref = ref, depth = undefined, map }:
    { ref: string, fullref?: string, depth?: number, map: Map<string, string> }):
  ...
  static async packedRefs({ fs, gitdir }: { fs: FileSystem, gitdir: string }) {
    const text = (await fs.read(`${gitdir}/packed-refs`, { encoding: 'utf8' })) as string
    const packed = GitPackedRefs.from(text)
    return packed.refs
  }
  ...
  static async listRefs(
    { fs, gitdir, filepath }:
    { fs: FileSystem, gitdir: string, filepath: string }) {
  ...
  static async listTags({ fs, gitdir }: { fs: FileSystem, gitdir: string }) {
    const tags = await GitRefManager.listRefs({
      fs,
      gitdir,
      filepath: `refs/tags`,
